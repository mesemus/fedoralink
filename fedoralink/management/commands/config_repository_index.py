...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for field in fields.values():
            ...
            if field.indexer_name.endswith('_ts'):
                # TODO: how to escape to make sure ^ and $ are there only once?
                ldpath.append(
                    """
                    {0} = fn:concat("^^^^", <{1}>, "$$$$") :: {2};
        ...
        ldpath.append('fedora_parent_id_ts = <http://fedora.info/definitions/v4/repository#hasParent> :: xsd:string;')

        ldpath.append('fedora_mixin_types_ts = "%s" :: xsd:string;' % ' '.join(model_classes))
